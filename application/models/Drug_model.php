...
class Drug_model extends CI_Model {
	...
	public function get_all_countries() {        
		return $this->db->select("*")->from("lab_drugs")->where("is_deleted", 'N')->get()->result_array();
    }

	public function getDrugHistory(){
		
		$sql = "select b.*, a.name as drugname from lab_drugs a, lab_drug_stocks_transaction b where a.id=b.drug_id ";
		return $this->db->query($sql)->result_array();
	}
	
	public function currentstock(){
		
		$sql = "select b.*, a.name as drugname from lab_drugs a, lab_drug_current_stocks b where a.id=b.drug_id ";
		return $this->db->query($sql)->result_array();
	}
	
	
	public function checkExists($name) {       
	   return $this->db->select("*")->from("lab_drugs")->where("name", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("lab_drugs")->where("id !=" ,$id)->where("name", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	
	public function save($data) {
			
		$this->db->insert('lab_drugs', $data);       
    }
	
	
	public function savestock($data) {
			
		$this->db->insert('lab_drug_stocks_transaction', $data);  

//update curent stock.
			$sql = "SELECT SUM(quantity) as stock from lab_drug_stocks_transaction where drug_id = '".$data["drug_id"]."'";
			$num = $this->db->query($sql)->num_rows();
			if($num)
			{
				$row = $this->db->query($sql)->row_array();
				$sql = "SELECT * from lab_drug_current_stocks where drug_id = '".$data["drug_id"]."'";
				$num2= $this->db->query($sql)->num_rows();
				if($num2)
				{
				$sql = "update lab_drug_current_stocks SET current_stock = '".$row["stock"]."' where drug_id = '".$data["drug_id"]."'";
				$this->db->query($sql);
				}
				else{				
				$sql = "INSERT INTO lab_drug_current_stocks SET current_stock = '".$row["stock"]."' , drug_id = '".$data["drug_id"]."'";
				$this->db->query($sql);
				}
			}
			else
	...
	{
		return $this->db->select('*')->from('lab_drugs')->where('id',$id)->get()->row_array();
	}
