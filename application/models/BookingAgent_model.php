<?php 
class BookingAgent_model extends CI_Model {
	...
	public function get_all_users($agent_id=null) {        
		//return $this->db->select("*")->from("lab_orders")->order_by('id', 'DESC')->get()->result_array();
		/*$sql = "select a.*,b.firstname as cfname, b.lastname as clname,c.name as agentname from lab_orders a INNER JOIN lab_customers b ON a.customer_id = b.id and a.assigned_to='".$agent_id."'  LEFT JOIN admin c ON c.admin_id = a.assigned_to  order by a.id desc";*/
		
		
		$sql = "(select a.*,a.id as orderid,b.firstname as cfname, b.lastname as clname,c.name as agentname from lab_orders a INNER JOIN lab_customers b ON a.customer_id = b.id and a.end_user_type= 'Customer' AND  a.assigned_to='".$agent_id."' LEFT JOIN admin c ON c.admin_id = a.assigned_to)";
		$sql .= " UNION ";
		 $sql .=" (select a.*,a.id as orderid,d.name as cfname, '' as clname,c.name as agentname from lab_orders a INNER JOIN lab_external_clients d ON a.customer_id = d.id and a.end_user_type= 'Lab'  AND  a.assigned_to='".$agent_id."' LEFT JOIN admin c ON c.admin_id = a.assigned_to) ";
