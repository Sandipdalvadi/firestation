<?php 
class ComboTestPkg_model extends CI_Model {
	...
	public function get_all_pkg() {        
		...
		$sql = "select a.*,b.name as clientname from lab_combotest_pkg a,lab_external_clients b where a.client_id =b.id AND a.parent_id = 0 AND a.is_deleted='N' ";
		return $this->db->query($sql)->result_array();
    }
	
	
	public function get_all_pkg_by_client_id($id) {        
		//return $this->db->select("*")->from("lab_test")->where("is_deleted", 'N')->get()->result_array();
		$sql = "select a.*,b.name as clientname from lab_combotest_pkg a,lab_external_clients b where a.client_id =b.id AND a.parent_id = 0 AND a.is_deleted='N' and b.id='".$id."'";
		return $this->db->query($sql)->result_array();
    }
	
		public function get_all_pkg_parent($client_id) {        
		//return $this->db->select("*")->from("lab_test")->where("is_deleted", 'N')->get()->result_array();
		$sql = "select a.*,b.name as clientname from lab_combotest_pkg a,lab_external_clients b where a.client_id =b.id AND a.parent_id = 0 AND a.is_deleted='N'  AND a.client_id='".$client_id."'";
		return $this->db->query($sql)->result_array();
    }
	...
	public function checkExists($title,$client_id=NULL) {       
	   return $this->db->select("*")->from("lab_combotest_pkg")->where("title", $title)->where("client_id", $client_id)->get()->num_rows();
    }
	public function checkExistsEdit($name,$id,$client_id=NULL) {       
	   return $this->db->select("*")->from("lab_combotest_pkg")->where("id !=" ,$id)->where("client_id", $client_id)->where("title", $name)->get()->num_rows();
    }
