<?php 
class Specialoffers_model extends CI_Model {
	...
	public function get_all_offers() {        
		return $this->db->select("*")->from("lab_special_offers")->where("is_deleted", 'N')->get()->result_array();
    }	
	public function checkExists($name) {       
	   return $this->db->select("*")->from("lab_special_offers")->where("coupon_code", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("lab_special_offers")->where("id !=" ,$id)->where("coupon_code", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	
	public function save($data) {
			
		$posteddata["coupon_code"] = $data["coupon_code"];
		//$posteddata["minimum_amount"] = $data["minimum_amount"];
		$posteddata["discount_value"] = $data["discount_value"];
		$posteddata["discount_mode"] = $data["discount_mode"];
		$posteddata["is_deleted"] = 'N';
		
		$daterange = explode("-",$data["daterange"]);
		
		$start_date = $daterange[0];
		$start_date = explode("/",trim($start_date));
		$posteddata["start_date"] = $start_date[2]."-".$start_date[0].'-'.$start_date[1];
		
		$end_date = $daterange[1];
		$end_date = explode("/",trim($end_date));
		$posteddata["end_date"] = $end_date[2]."-".$end_date[0].'-'.$end_date[1];
		
		$this->db->insert('lab_special_offers', $posteddata);       
    }
	...
	$this->db->where('id',$id);
	$this->db->update('lab_special_offers', $data);
    }
	public function update($data)
	{
		...
		unset($data['cid']);		
		$this->db->update('lab_special_offers', $posteddata);
	}
	public function getdataById($id=null)
	{
		return $this->db->select('*')->from('lab_special_offers')->where('id',$id)->get()->row_array();
	}
