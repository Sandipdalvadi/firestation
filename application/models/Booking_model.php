...
class Booking_model extends CI_Model {
	...
	public function get_all_users() {        
		//return $this->db->select("*")->from("lab_orders")->order_by('id', 'DESC')->get()->result_array();
		/*$sql = "select a.*,b.firstname as cfname, b.lastname as clname,c.name as agentname from lab_orders a INNER JOIN lab_customers b ON a.customer_id = b.id and a.end_user_type= 'Customer'  LEFT JOIN admin c ON c.admin_id = a.assigned_to   ";
		$sql .= " UNION ";
		$sql .= "select a.*,d.name as cfname, '' as clname,c.name as agentname from lab_orders a INNER JOIN lab_external_clients d ON a.customer_id = d.id  and a.end_user_type= 'Lab'  LEFT JOIN admin c ON c.admin_id = a.assigned_to  ";*/
		
		
		$sql = "(select a.*,a.id as orderid,b.firstname as cfname, b.lastname as clname,c.name as agentname from lab_orders a INNER JOIN lab_customers b ON a.customer_id = b.id and a.end_user_type= 'Customer' LEFT JOIN admin c ON c.admin_id = a.assigned_to)";
		$sql .= " UNION ";
		 $sql .=" (select a.*,a.id as orderid,d.name as cfname, '' as clname,c.name as agentname from lab_orders a INNER JOIN lab_external_clients d ON a.customer_id = d.id and a.end_user_type= 'Lab' LEFT JOIN admin c ON c.admin_id = a.assigned_to) ";
	...
	{
		return $this->db->select('*')->from('lab_orders')->where('id',$id)->get()->row_array();
	}
