...
class Occupation_model extends CI_Model {
	...
	public function get_all_countries() {        
		return $this->db->select("*")->from("lab_occupation")->where("is_deleted", 'N')->get()->result_array();
    }	
	public function checkExists($name) {       
	   return $this->db->select("*")->from("lab_occupation")->where("name", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("lab_occupation")->where("id !=" ,$id)->where("name", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	
	public function save($data) {
			
		$this->db->insert('lab_occupation', $data);       
    }
	...
	$this->db->where('id',$id);
	$this->db->update('lab_occupation', $data);
    }
	...
	{
		return $this->db->select('*')->from('lab_occupation')->where('id',$id)->get()->row_array();
	}
