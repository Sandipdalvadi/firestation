<?php 
class Gallerycat_model extends CI_Model {
	...
	public function get_all_countries2() {        
		return $this->db->select("*")->from("gallery_category")->where("is_deleted", 'N')->where('parent_id','0')->get()->result_array();
    }
	public function checkExists($name) {       
	   return $this->db->select("*")->from("gallery_category")->where("title", $name)->where('parent_id','0')->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("gallery_category")->where("id !=" ,$id)->where("title", $name)->where('parent_id','0')->where("is_deleted", 'N')->get()->num_rows();
    }
