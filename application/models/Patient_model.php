...
class Patient_model extends CI_Model {
	...
	public function get_all_countries() {        
		return $this->db->select("*")->from("lab_customers")->where("is_deleted", 'N')->get()->result_array();
		//$sql = "select * from lab_customers a,lab_customers_contact_details b where a.id=b.customer_id";
		return $this->db->query($sql)->result_array();
    }	
	public function checkExists($name) {       
	   return $this->db->select("*")->from("lab_customers")->where("ic_or_pp_name_username", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("lab_customers")->where("id !=" ,$id)->where("ic_or_pp_name_username", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	
	public function save($data) {
			
		$this->db->insert('lab_customers', $data);       
    }
	...
	$this->db->where('id',$id);
	$this->db->update('lab_customers', $data);
    }
	...
	{
		return $this->db->select('*')->from('lab_customers')->where('id',$id)->get()->row_array();
	}
