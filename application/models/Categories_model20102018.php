...
class Categories_model extends CI_Model {
	...
	public function view_order($order=NULL){
        $query = $this->db->query("select u.first_name,u.last_name,u.mobile,u.email_id,u.address1,u.address2,u.city,od.order_no,od.pro_qty,od.pro_price,od.order_total,od.order_status,od.dateCreated,p.pro_name from orders as od right join users as u ON od.user_id=u.user_id right join products as p ON od.pro_id=p.prod_id WHERE od.order_no!='' AND od.order_no='$order'");
        return $query->result_array();
	}
	...
	public function get_vendor_order_by_id($order_id=NULL){
        $query = $this->db->query("select od.order_no,od.pro_qty,od.pro_price,od.order_total,od.order_status,od.dateCreated,od.order_id,p.pro_name,od.order_remarks,od.message,od.size,od.delivery_date,od.delivery_time,od.specials,od.order_address,ua.user_id,ua.first_name,ua.last_name,ua.email_id,ua.mobile,ua.address1,ua.address2,ua.landmark,ua.city from orders as od right join products as p ON od.pro_id=p.prod_id right join user_addresses as ua ON od.order_address=ua.ua_id WHERE od.order_id='$order_id' AND od.order_status!=5");
