...
class District_model extends CI_Model {
	...
	public function get_all() {        
		...
		return $this->db->select('lab_district.*, lab_country.name as country, lab_state.name as state')
         ->from('lab_district')
		  ->join('lab_country', 'lab_district.country_id = lab_country.id')		      
		  ->join('lab_state', 'lab_district.state_id = lab_state.id')
		 ->where("lab_district.is_deleted", 'N')->get()->result_array();

    }	
	
	public function get_all_dis_by_stateid($state_id) { 
	
		return $this->db->select("*")->from("lab_district")->where("state_id", $state_id)->where("is_deleted", 'N')->get()->result_array();
	}
	

	
	public function checkExists($name,$state_id) {       
	   return $this->db->select("*")->from("lab_district")->where("name", $name)->where("is_deleted", 'N')->where("state_id", $state_id)->get()->num_rows();
    }
	public function checkExistsEdit($name,$id,$state_id) {       
	   return $this->db->select("*")->from("lab_district")->where("id !=" ,$id)->where("is_deleted", 'N')->where("name", $name)->where("state_id", $state_id)->get()->num_rows();
    }
	
	public function save($data) {		
		$this->db->insert('lab_district', $data);       
    }
	...
	$this->db->where('id',$id);
	$this->db->update('lab_district', $data);
    }
	...
	{
		return $this->db->select('*')->from('lab_district')->where('id',$id)->get()->row_array();
	}
