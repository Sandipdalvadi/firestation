...
class State_model extends CI_Model {
	...
	public function get_all() {        
		...
		return $this->db->select('lab_state.*, lab_country.name as country')
         ->from('lab_state')
         ->join('lab_country', 'lab_state.country_id = lab_country.id')->where("lab_state.is_deleted", 'N')->get()->result_array();

    }
	public function get_all_state_by_countries($country_id) { 
	
		return $this->db->select("*")->from("lab_state")->where("country_id", $country_id)->where("is_deleted", 'N')->get()->result_array();
	}



	
	public function checkExists($name,$country_id) {       
	   return $this->db->select("*")->from("lab_state")->where("name", $name)->where("is_deleted", 'N')->where("country_id", $country_id)->get()->num_rows();
    }
	public function checkExistsEdit($name,$id,$country_id) {       
	   return $this->db->select("*")->from("lab_state")->where("id !=" ,$id)->where("name", $name)->where("is_deleted", 'N')->where("country_id", $country_id)->get()->num_rows();
    }
	
	public function save($data) {		
		$this->db->insert('lab_state', $data);       
    }
	...
	{
		return $this->db->select('*')->from('lab_state')->where('id',$id)->get()->row_array();
	}
