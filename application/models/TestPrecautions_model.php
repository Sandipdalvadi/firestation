<?php 
class TestPrecautions_model extends CI_Model {
	...
	public function get_all_precautions() {        
		return $this->db->select("*")->from("lab_test_precautions")->where("is_deleted", 'N')->get()->result_array();
    }	
	public function checkExists($name) {       
	   return $this->db->select("*")->from("lab_test_precautions")->where("name", $name)->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("lab_test_precautions")->where("id !=" ,$id)->where("name", $name)->get()->num_rows();
    }
	
	public function save($data) {		
		$this->db->insert('lab_test_precautions', $data);       
    }
	...
	$this->db->where('id',$id);
	$this->db->update('lab_test_precautions', $data);
    }
	...
	{
		return $this->db->select('*')->from('lab_test_precautions')->where('id',$id)->get()->row_array();
	}
