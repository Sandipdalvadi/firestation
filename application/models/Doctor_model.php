...
class Doctor_model extends CI_Model {
	...
	public function get_all_countries() {        
		return $this->db->select("*")->from("lab_doctors")->where("is_deleted", 'N')->where("role", 'Doctor')->get()->result_array();
    }	
	public function get_all_countries2() {        
		return $this->db->select("*")->from("lab_doctors")->where("is_deleted", 'N')->where("role", 'Staff')->get()->result_array();
    }
	public function get_all_countries3() {        
		return $this->db->select("*")->from("lab_doctors")->where("is_deleted", 'N')->where("role", 'Admin')->get()->result_array();
    }
public function get_all_countries4() {        
		return $this->db->select("*")->from("lab_doctors")->where("is_deleted", 'N')->where("role", 'Superadmin')->get()->result_array();
    }	
	public function checkExists($name) {       
	   return $this->db->select("*")->from("lab_doctors")->where("ic_or_pp_name_username", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("lab_doctors")->where("id !=" ,$id)->where("ic_or_pp_name_username", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	
	public function save($data) {
			
		$this->db->insert('lab_doctors', $data);       
    }
	...
	$this->db->where('id',$id);
	$this->db->update('lab_doctors', $data);
    }
	...
	{
		return $this->db->select('*')->from('lab_doctors')->where('id',$id)->get()->row_array();
	}
