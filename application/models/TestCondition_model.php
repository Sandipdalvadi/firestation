<?php 
class TestCondition_model extends CI_Model {
	...
	public function checkExists($name) {       
	   return $this->db->select("*")->from("lab_country")->where("name", $name)->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("lab_country")->where("id !=" ,$id)->where("name", $name)->get()->num_rows();
    }
	
	public function save($data) {		
		$this->db->insert('lab_country', $data);       
    }
	...
	$this->db->where('id',$id);
	$this->db->update('lab_country', $data);
    }
	...
	{
		return $this->db->select('*')->from('lab_country')->where('id',$id)->get()->row_array();
	}
