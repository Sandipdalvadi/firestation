...
class Staff_model extends CI_Model {
	...
	public function get_all_countries() {        
		return $this->db->select("*")->from("admin_users")->where("is_deleted", 'N')->where("role", 'Treasurer')->get()->result_array();
    }	
	public function get_all_countries2() {        
		return $this->db->select("*")->from("admin_users")->where("is_deleted", 'N')->where("role", 'Staff')->get()->result_array();
    }
	public function get_all_countries3() {        
		return $this->db->select("*")->from("admin_users")->where("is_deleted", 'N')->where("role", 'Admin')->get()->result_array();
    }
public function get_all_countries4() {        
		return $this->db->select("*")->from("admin_users")->where("is_deleted", 'N')->where("role", 'Superadmin')->get()->result_array();
    }	
	public function checkExists($name) {       
	   return $this->db->select("*")->from("admin_users")->where("email", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("admin_users")->where("id !=" ,$id)->where("ic_or_pp_name_username", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	...
	{
		return $this->db->select('*')->from('admin_users')->where('id',$id)->get()->row_array();
	}
