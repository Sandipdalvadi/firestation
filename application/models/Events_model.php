...
class Events_model extends CI_Model {
	...
	public function get_all_events() {        
		return $this->db->select("*")->from("events")->where("is_deleted", 'N')->get()->result_array();
    }
	public function checkExists($name) {       
	   return $this->db->select("*")->from("events")->where("title", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("events")->where("id !=" ,$id)->where("title", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	...
	{
		return $this->db->select('*')->from('events')->where('id',$id)->get()->row_array();
	}
