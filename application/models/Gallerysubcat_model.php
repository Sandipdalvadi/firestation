<?php 
class Gallerysubcat_model extends CI_Model {
	...
	public function get_all_countries2() {        
		return $this->db->select("*")->from("gallery_category")->where("is_deleted", 'N')->get()->result_array();
    }
	public function checkExists($name,$parent_id) {       
	   return $this->db->select("*")->from("gallery_category")->where("parent_id", $parent_id)->where("title", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id,$parent_id) {       
	   return $this->db->select("*")->from("gallery_category")->where("parent_id" ,$parent_id)->where("id !=" ,$id)->where("title", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	...
	{
		return $this->db->select('*')->from('gallery_category')->where('id',$id)->get()->row_array();
	}
