<?php 
class Sponsershiptype_model extends CI_Model {
	...
	public function get_all_countries() {        
		return $this->db->select("*")->from("sponsership")->where("is_deleted", 'N')->get()->result_array();
    }	
...
public function get_all_countries4() {        
	...
	public function checkExists($name) {       
	   return $this->db->select("*")->from("sponsership")->where("name", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("sponsership")->where("id !=" ,$id)->where("name", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	
	public function save($data) {		
		$this->db->insert('sponsership', $data);       
    }
	...
	$this->db->where('id',$id);
	$this->db->update('sponsership', $data);
    }
	...
	{
		return $this->db->select('*')->from('sponsership')->where('id',$id)->get()->row_array();
	}
