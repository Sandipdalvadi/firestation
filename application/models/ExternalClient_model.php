<?php 
class ExternalClient_model extends CI_Model {
	...
	public function get_all_clients() {        
		return $this->db->select("*")->from("lab_external_clients")->where("is_deleted", 'N')->get()->result_array();
    }
	public function otherinfo() {        
		return $this->db->select("*")->from("lab_external_clients_metadata")->where("is_deleted", 'N')->get()->result_array();
    }	
	public function checkExists($name) {       
	   return $this->db->select("*")->from("lab_external_clients")->where("name", $name)->get()->num_rows();
    }
	public function checkotherinfoExists($name) {       
	   return $this->db->select("*")->from("lab_external_clients_metadata")->where("client_name", $name)->get()->num_rows();
    }
	public function checkExistsEmail($name) {       
	   return $this->db->select("*")->from("lab_external_clients")->where("email", $name)->get()->num_rows();
    }
	public function checkExistsContactno($name) {       
	   return $this->db->select("*")->from("lab_external_clients")->where("contactno", $name)->get()->num_rows();
    }
	...
	public function save($data) {
		$data["org_password"] = $data["password"];
         $data["password"] = md5($data["password"]);
		$this->db->insert('lab_external_clients', $data);       
    }
	public function saveotherinfo($data) {
		
		//echo "Here";
		//print_r($data);
		$test_data['client_name'] = $data['client_name'];
		$test_data['certifications'] = $data['certifications'];
		$test_data['rating'] = $data['rating'];
		//$test_data['parent_id'] =0;
		...
		$this->db->insert('lab_external_clients_metadata', $test_data);
	...
	$this->db->where('id',$id);
	$this->db->update('lab_external_clients', $data);
    }
	
	public function otherstatus($id) {	   
		   $data = array( 
		'is_deleted'      => 'Y' 
		);
	$this->db->where('id',$id);
	$this->db->update('lab_external_clients_metadata', $data);
    }
	...
	{
		return $this->db->select('*')->from('lab_external_clients')->where('id',$id)->get()->row_array();
	}
	public function getotherdataById($id=null)
	{
		return $this->db->select('*')->from('lab_external_clients_metadata')->where('id',$id)->get()->row_array();
	}
	...
	public function updateotherinfo($data){
		...
		$this->db->where('id',$data['cid']);		
		$this->db->update('lab_external_clients_metadata', $test_data);
	}
