<?php 
class Testtype_model extends CI_Model {
	
	function __construct() {
		parent::__construct();
		$this->load->library('session');
		$this->load->database();
		//ALTER TABLE `lab_test` CHANGE `test_type` `test_type` INT NOT NULL;
		//ALTER TABLE `lab_test_by_conditions` CHANGE `test_type` `test_type` INT NOT NULL;
	}
	
	public function get_all_countries() {        
		return $this->db->select("*")->from("lab_test_type")->where("is_deleted", 'N')->get()->result_array();
    }	
	public function checkExists($name) {       
	   return $this->db->select("*")->from("lab_test_type")->where("name", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	public function checkExistsEdit($name,$id) {       
	   return $this->db->select("*")->from("lab_test_type")->where("id !=" ,$id)->where("name", $name)->where("is_deleted", 'N')->get()->num_rows();
    }
	
	public function save($data) {
			
		$this->db->insert('lab_test_type', $data);       
    }
	...
	$this->db->where('id',$id);
	$this->db->update('lab_test_type', $data);
    }
	...
	{
		return $this->db->select('*')->from('lab_test_type')->where('id',$id)->get()->row_array();
	}
