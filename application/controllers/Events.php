...
if(($this->session->userdata('loginUser')!=''))
	...
	public function add()
	{
		...
		$sql = "select * from sponsership where is_deleted='N'";
		$data["sp"] = $this->db->query($sql)->result_array();
	...
	public function save()
	{
		...
		$reportsdata['location'] = $data['location'];
		...
		else if( $data['event_type'] == "Paid")
		{
			$reportsdata['price'] = $data['price'];
		}
		else if( $data['event_type'] == "Sponsorship")
		{
			$reportsdata['price'] = 0;
			$reportsdata['sponsorship'] = implode(",",$data['sponsorship']);
		...
		$reportsdata["event_date"] = $event_date1[2]."-".$event_date1[0].'-'.$event_date1[1]." ".$event_date_Arr[1];
