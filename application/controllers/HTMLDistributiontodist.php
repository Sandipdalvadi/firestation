            ...
            <tbody>
			  ...
			  */
              ...
              <tr><form action="<?php echo base_url()?>distributiontodist/save" method="post">  <input type="hidden" name="ID" value="<?php echo $row['stock_id']; ?>">
                <td>	<select required style="width:140px;" class="form-control show-tick select2" name="item_id" >  
...
<select style="width:140px;" required class="form-control show-tick select2" name="district" >  
   ...
   <td><input type="text" required class="form-control" placeholder="Discription" value="<?php echo $row['description']; ?>"  name="description"></td>
                 ...
                 <td><input type="text"  required class="form-control" placeholder="D. ORDER NO" value="<?php //echo $row['distribution_order_no']; ?>" name="distribution_order_no"></td>
                <td><input type="text" required class="form-control" placeholder="Qty " value="<?php //echo $row['qty_recieved']; ?>" required name="qty_recieved" id="qty_recieved" onchange="check()">
            ...
            <tbody>
			  ...
			  $totalInvoices = mysql_query("SELECT * FROM recivestockbytender WHERE updated_by = '".$_SESSION["name"]."' AND transfer = 1  ORDER BY stock_id DESC");
				   ...
				   */
				   $count = 0;
					$tot = 0;
				   	$sql = "SELECT a.*,b.unit_name,c.logistics_name FROM recivestockbytender_new a ,units_masters b  ,  logistics_kits_masters c where c.id=a.item_id and  a.to_transfer_id=b.id";
					$totalInvoices = $this->db->query($sql)->num_rows();
					if($totalInvoices > 0)
					{
						$rows = $this->db->query($sql)->result_array();
