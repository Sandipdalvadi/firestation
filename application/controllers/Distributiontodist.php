...
class distributiontodist extends CI_Controller { 
	...
	public function save()
	{
	    ...
	    $main_total_stock =  $pending_main_stock;
	    $main_stock = $main_total_stock - $_POST['qty_recieved'];
	    
	    //district current stock
	    $sql = "select dist_current_stock from recivestockbytender_new where item_id='".$itemId."' and ( (to_transfer_id='".$_POST["district"]."' AND to_transfer_type='District') OR (from_transfer_id='".$_POST["district"]."' AND from_transfer_type='District') ) order by stock_id desc limit 1";
	    $num2 = $this->db->query($sql)->num_rows();
	    $dist_current_stock = 0;
	    if($num2 >=1 )
	    {
	         $row2 = $this->db->query($sql)->row_array();
	         $dist_current_stock = $row2['dist_current_stock'];
	    }
	    $dist_current_stock = $dist_current_stock + $_POST['qty_recieved'];
	    $dist_receive = $_POST['qty_recieved'];
	    
	    
	    
	    //get tender id.
	    
	    $sql = "select * from tenders_details_new_details where item_id='".$_POST['item_id']."' AND item_cat='Logistics' order by id desc limit 1 ";
	    $num2 = $this->db->query($sql)->num_rows();
	    $tender_id_st = NULL;
	    if($num2 >=1 )
	    {
	         $row2 = $this->db->query($sql)->row_array();
	         $tender_id_st = $row2['tenders_details_new_id'];
	    }
