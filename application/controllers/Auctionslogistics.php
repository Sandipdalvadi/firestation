...
class Auctionslogistics extends CI_Controller { 
	...
	public function index($id=0)
	{
		...
		$this->load->view('auctions_logistics',$data);
...
if (move_uploaded_file($_FILES["challan"]["tmp_name"], $target_filechallan)) {
    ...
    $arr = $_POST['items'];
    foreach ($arr as $id) {
 $sql ="UPDATE unserviceable_list SET auction = 1,condemnation=2 WHERE id = ".$id."";
 $this->db->query($sql);
 
  $sql ="UPDATE unserviceable_list SET disbursed = 1, lot_number = '".$_POST['allottid']."', challan = '$target_filechallan', auction_amount = '".$_POST['auction_amount']."', treasury_deposit_details = '".$_POST['treasury_deposit_details']."'  WHERE id = ".$id."";
  $this->db->query($sql);
    }
