 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1> Additional Quoata Allotment (Units) </h1>
      <ol class="breadcrumb">
  ...
  <div class="row">
            ...
            <tbody>
			  ...
			  /*$totalInvoices = mysql_query("SELECT * FROM fuel_quota_allotments WHERE district = '' ORDER BY id DESC");
				   ...
				   $count = 0;
					...
					$sql = "SELECT a.*,b.unit_name as districtsss FROM fuel_quota_allotments a LEFT JOIN units_masters b on a.district=b.id  WHERE a.district IS NOT NULL AND a.allotments_type = 'additional' ORDER BY a.id DESC";
              ...
              <tr>
                 ...
                 <td><?php echo $row['districtsss']; ?></td>
                <td><?php echo $row['quota_liters']; ?></td>
                <td><?php echo date("d/m/Y", strtotime($row['created_at'])); ?></td>
               
			   <td><a href="<?php echo base_url()?>Quotaallotmentdistrictadd/index/<?php echo $row['id']; ?>"><img src="<?php echo base_url()?>assets/img/icon-edit.png" height="20"></a></td>
    ...
    <div class="col-sm-6" style="background-color:#d8e1f0;"><form action="<?php echo base_url()?>Quotaallotmentdistrictadd/save" method="post" enctype="multipart/form-data">
            	
				<div class="form-group row">&nbsp;</div>
				<div class="form-group row">
				  <label for="example-search-input" class="col-sm-4 col-form-label">District/Unit</label>
				  <div class="col-sm-6">
				      
 
					<input type="hidden" name="id" value="<?php echo $id?>">
				<select id="district" class="myselect form-control  show-tick" name="district" required> 
				   ...
				   //while($row = mysqli_fetch_array($totalInvoices)) {
					foreach($rows as $key=>$rowMakes){				
							...
							?>		 
							<option value="<?php echo $rowMakes['id'];?>" <?php if (!empty($fetch222['district'])) { if($fetch222['district']== $rowMakes['id']) { echo 'selected'; } } ?> ><?php echo $rowMakes['unit_name'];?></option>			 
							<?php  }
							}else{
							echo '<option value="">Makes Not Available</option>';
							} ?>
