            ...
            <tbody>
			  ...
			  */
				   ...
				   while($row = mysql_fetch_array($totalInvoices)) { */
				   	...
				   	$sql = "SELECT * FROM recivestockbytender  WHERE item_name <> ''  ORDER BY stock_id DESC";
				   ...
				   $count++;
				    ...
				    if($row['qty_recieved'] > 0 && $row['item_name'] != '')
				    {
				   ?>
              <tr><form action="<?php echo base_url()?>distributiontodist/save" method="post">  <input type="hidden" name="ID" value="<?php echo $row['stock_id']; ?>">
                <td>	<select style="width:200px;" class="form-control show-tick" name="item_name" id="item_name">  
...
$sql = "SELECT * FROM logistics_kits_masters WHERE logistics_name LIKE '%".$row['item_name']."%'";
					$totalInvoices = $this->db->query($sql)->num_rows();
					if($totalInvoices > 0)
					{
						$rows = $this->db->query($sql)->result_array();
                ...
                <td>	<select style="width:200px;" class="form-control show-tick" name="district" id="district">  
				   ...
				   //while($row = mysqli_fetch_array($totalInvoices)) {
					foreach($rows as $key=>$rowMakes){
						?>		 
						<option value="<?php echo $rowMakes['unit_name'];?>" <?php if(!empty($row['district'])) { if ($rowMakes['unit_name']==$row['district']) { echo 'selected'; } } ?>><?php echo $rowMakes['unit_name'];?></option>			 
                        <?php  }
                        }else{
                        echo '<option value="">Makes Not Available</option>';
                        } ?>
                        </select></td>
               <td><input type="text" class="form-control" placeholder="IV No" value="<?php echo $row['description']; ?>" id="description" name="description"></td>
               
               
                 <td><input type="text" class="form-control" placeholder="IV No" value="<?php //echo $row['distribution_order_no']; ?>" name="distribution_order_no"></td>
                <td><input type="text" class="form-control" placeholder="Qty " value="<?php //echo $row['qty_recieved']; ?>" required name="qty_recieved" id="qty_recieved" onchange="check()">
                ...
                <input type="hidden" class="form-control" placeholder="IV No" value="<?php echo $row['qty_recieved']; ?>" name="qty_recieved_old" id="qty_recieved_old">
                  ...
                  <td><input type="text" class="form-control" placeholder="IV No" value="<?php echo $row['iv_number']; ?>" required name="iv_number"></td>
                  <td>	<input type="date" class="form-control" placeholder="IV Date" value="<?php echo $row['iv_date']; ?>" name="iv_date"></td>
