<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <form action="<?php echo base_url()?>Recivestockbytendernew/save" method="post" enctype="multipart/form-data">
...
?>
<select disabled name="financial_year" required  class="form-control select2">
			 ...
			 /* if(!empty($_SESSION['district']))
			  ...
			  $totalInvoices = mysql_query("SELECT * FROM tenders_details WHERE financial_year <> ''  ORDER BY id DESC");
							...
							*/
							...
							$sql = "SELECT a.*,b.logistics_name FROM tenders_details_new_details a LEFT JOIN logistics_kits_masters b on a.item_id=b.id WHERE tenders_details_new_id ='".$fetch222['id']."' and a.item_cat='Logistics' ORDER BY id DESC";
					$totalInvoices = $this->db->query($sql)->num_rows();
...
$sql = "SELECT * FROM logistics_licofficers_masters";
				   ...
				   //while($row = mysqli_fetch_array($totalInvoices)) {
					foreach($rows as $key=>$rowMakes){									?>		 
						<option value="<?php echo $rowMakes['id'];?>" <?php if(!empty($fetch222['lci_officer'])) { if ($fetch222['lci_officer_1']==$rowMakes['id']) { echo 'selected'; } } ?>><?php echo $rowMakes['lci_officer'];?></option>			 
                        <?php  }
                        }else{
				   ...
				   //while($row = mysqli_fetch_array($totalInvoices)) {
					foreach($rows as $key=>$rowMakes){
							...
							?>		 
						<option value="<?php echo $rowMakes['id'];?>" <?php if(!empty($fetch222['lci_officer'])) { if ($fetch222['lci_officer_2']==$rowMakes['id']) { echo 'selected'; } } ?>><?php echo $rowMakes['lci_officer'];?></option>			 
                        <?php  }
                        }else{
                        echo '<option value="">Not Available</option>';
                        } ?>
				  ...
				  LCI Officer 3
				   ...
				   //while($row = mysqli_fetch_array($totalInvoices)) {
					foreach($rows as $key=>$rowMakes){
							?>		 
						<option value="<?php echo $rowMakes['id'];?>" <?php if(!empty($fetch222['lci_officer'])) { if ($fetch222['lci_officer_3']==$rowMakes['id']) { echo 'selected'; } } ?>><?php echo $rowMakes['lci_officer'];?></option>			 
                        <?php  }
                        }else{
                        echo '<option value="">Not Available</option>';
                        } ?>
 ...
 <div class="col-sm-4">
				Distribution Order			  
				   ...
				   <div class="col-sm-4">
				    ...
				    <div class="col-sm-4"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				    <input type="text" class="form-control" placeholder="Reason for reject " value="<?php echo $fetch222['notes']; ?>" name="notes">	<button name="recivestockbytender" value="reject" type="submit"><img src="<?php echo base_url()?>assets/img/reject.png" height="30">Reject</button>
				  </div>
