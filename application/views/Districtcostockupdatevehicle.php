 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h2> District Stock Update <small>Vehicle </small> <small>(Receive Stock) </small> </h2>
      <ol class="breadcrumb">
                  ...
                  $totalInvoices = mysql_query("SELECT * FROM recivestockbytender  WHERE district <> '' ORDER BY stock_id DESC"); 
				   ...
				   $count = 0;
					$tot = 0;
						...
						$sql = "select a.*,u.unit_name as disttts,u.id as diid,c.purpose from vehicle_allotment a LEFT JOIN vehicleD b on a.vehicleno=b.vhno

						LEFT JOIN tenders_details_new c ON c.tender_number=b.tender_number
						LEFT JOIN units_masters u ON u.id=a.unit_to

						where 1 and a.rv_number IS NULL
						";
...
//					
              ...
              <tr><form action="<?php echo base_url()?>Districtcostockupdatevehicle/save" method="post">  <input type="hidden" name="ID" value="<?php echo $row['allotment_id']; ?>">
               <td style="width:200px;"><?php echo $row['vehicleno']; ?></td>
                ...
                <!--<td><?php echo $row['qty_recieved']; ?></td>-->
                 ...
                 <td><input type="text" class="form-control" required placeholder="RV No" value="<?php echo $row['rv_number']; ?>"  <?php if(!empty($row['rv_number'])){ ?>  <?php } ?> name="rv_number"></td>
                  <td>	<input type="date" class="form-control" placeholder="RV Date" value="<?php echo $row['rv_date']; ?>"  <?php if(!empty(rv_date)){ ?>  <?php } ?> name="rv_date"></td>
