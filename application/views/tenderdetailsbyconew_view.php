...
?>
				...
				<label for="example-search-input" class="col-sm-4 col-form-label">RC NO</label>
				   ...
				   <div class="col-sm-6">
				       <input type="text" required class="form-control" placeholder="Supply Order No" value="<?php echo $fetch222['supply_order_number']; ?>" name="supply_order_number">
				       </div>-->
            ...
            <tbody>
			 ...
			 /* if(!empty($_SESSION['district']))
			  ...
			  $totalInvoices = mysql_query("SELECT * FROM tenders_details WHERE financial_year <> ''  ORDER BY id DESC");
							...
							*/
							$sql = " select * from ( ";
							$sql .= "SELECT a.*,b.logistics_name FROM tenders_details_new_details a INNER JOIN logistics_kits_masters b on a.item_id=b.id WHERE tenders_details_new_id ='".$fetch222['id']."' and a.item_cat='Logistics' ";
							
							$sql .= " UNION ";
							
							$sql .= "SELECT a.*,b.vehicle_category as logistics_name FROM tenders_details_new_details a INNER JOIN vehicle_category_masters b on a.item_id=b.id WHERE tenders_details_new_id ='".$fetch222['id']."' and a.item_cat='Vehicle' ";
